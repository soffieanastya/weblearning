...
class soal_pemanasan extends CI_Controller {
	...
	public function aksi_potensial(){
		...
		if($cekjawaban == "netral"){
			...
			// pokoknya, kalau id soal yg ada di database lebih besar dari id soal dan status di db lebih besar dari status disini, berarti gausah di update. kalau sama dua-duana, berarti gausah di update juga
			if($ceksoal->id_soal <= $id_soal){
				if($ceksoal->status < $status ){
					//update
	...
	public function aksi_arus(){
		...
		if($cekjawaban == "bolak-balik" || $cekjawaban == "bolak balik" || $cekjawaban == "direct current" || $cekjawaban == "DC"){
			//masukin ke database
			$id_siswa = $this->session->userdata('id_siswa');
			$id_soal  = 1;
			$status   = 3;
	...
	public function aksi_resistor(){
		...
		if($cekjawaban == "menghambat"){
			...
			// pokoknya, kalau id soal yg ada di database lebih besar dari id soal dan status di db lebih besar dari status disini, berarti gausah di update. kalau sama dua-duana, berarti gausah di update juga
			if($ceksoal->id_soal <= $id_soal){
				if($ceksoal->id_soal == $id_soal and $ceksoal->status < $status){
					//update
					$this->model_siskom->update_pemanasan($data,$where,'soal_pemanasan');
				}else if($ceksoal->id_soal < $id_soal){
					$this->model_siskom->update_pemanasan($data,$where,'soal_pemanasan');
				}
			}
			redirect ('materi');
	...
	public function aksi_transistor(){
		...
		if($cekjawaban == "tiga" || $cekjawaban == "3"){
			//masukin ke database
			$id_siswa = $this->session->userdata('id_siswa');
			$id_soal  = 2;
			$status   = 5;
	...
	public function aksi_transformator(){
		...
		if($cekjawaban == "dua" || $cekjawaban == "2"){
			//masukin ke database
			$id_siswa = $this->session->userdata('id_siswa');
			$id_soal  = 2;
			$status   = 6;
